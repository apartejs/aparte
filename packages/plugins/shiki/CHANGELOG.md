...
- f91584f: A light/dark theme pair now follows the OS colour scheme when the page sets no `data-aparte-theme`, the way core's own theme does; nothing to change on your side.
...
- a57ad06: `theme` accepts a `{ light, dark }` pair on both entries; the code block then follows core's `[data-aparte-theme="dark"]` switch. A single theme works exactly as before.
