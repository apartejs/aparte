...
- 4a73996: A streamed code fence now gets `class="language-<name>"`, the same class a one-shot renderer writes — so model text can no longer wear core's own `aparte-*` names while a message streams. Nothing to change in your code.
