/**
 * conversation/compactor.ts — the context-window budget.
 *
 ...
 * what `createCompactionSelector` (selector.ts) walks the history against, and what
 * `<aparte-context>` in core draws as a gauge.
 *
 ...
 * window of verbatim turns. What is NOT here any more (since 0.16.0): an assembler that
 * rebuilt a message list with a summary and RAG-retrieved excerpts. It had no caller in
 * this repository and no producer for its excerpts — a page documented it as "Ready"
 * for nobody — and the summarising itself lives where the transport is, in core's
 * `AparteClient.compact()`. Budgeting is the engine's; the rest was another product's.
 *
 * Browser-portable: zero deps, a char-count heuristic.
 */
