 ...
 *   /guides/tools/~5200                        the viewport, scrolled 5200px down
...
for (const t of targets) {
    const m = t.match(/^([^@#:~]+)(?:@(\d+))?(?:~(\d+))?(?:#([^:]+))?(?::(light|dark))?$/);
    if (!m) { console.log('cible illisible:', t); continue; }
    const [, route, w, scrollY, sel, theme] = m;
    ...
    const name = slug(route + (w ? '-' + w : '') + (scrollY ? '-y' + scrollY : '') + (sel ? '-' + sel : '') + (theme ? '-' + theme : ''));
    ...
    if (sel) {
        ...
        // A scroll offset asks for THAT screen, so full-page would ignore what was asked for.
        await page.screenshot({ path: file, fullPage: !scrollY });
    }
    if (ok) {
        // eslint-disable-next-line no-undef -- runs in the page, not in Node
