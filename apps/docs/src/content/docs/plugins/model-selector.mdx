...
import AparteModelSelectorApi from '../../../generated/element-api/aparte-model-selector.mdx';
...
subpath per framework. Import that subpath once, anywhere in your app, and the tag is
checked like any other: an unknown attribute or a wrong value type is a compile error.
...
A **directive** rather than a type, because Angular's template compiler needs a class
...
Outside Angular a presence attribute is `''` to set and `null` to remove, never `false` —
see [the rule](/frameworks/elements/#the-one-thing-to-know-about-presence-attributes).
