...
is inlined in no mode: aparté sends nothing for it and says so once with a `console.warn` naming
the file, while the chip stays on screen for your own upload or RAG layer to act on.
