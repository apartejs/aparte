...
:::caution[Beta]
Every `@aparte/*` package is a plain `0.x`. Since 0.17.0 the declared surface — elements,
attributes, events, exports, locale keys, tokens — is [frozen](/reference/stability/): a
name leaves it only through a notice release. Below that surface a rename still lands as a
rename rather than behind a deprecated alias, so a minor can ask you to change a line — pin
an exact version if that matters to you, and read
...
`@aparte/engine` bundled into core's. One package asks for a little more:
[`@aparte/provider-transformers`](/providers/ai/transformers/) runs its inference in a
worker, and a worker reads no import map of its own — its page says how the two meet.
...
your side — `@beta`, `@alpha`, `@0.13`, `@latest` — is a different URL, and a different URL is a
...
:::note[A static shell wants the stylesheet in the head]
Importing the stylesheet from JavaScript is fine when the framework renders your page:
the markup and the styles arrive together. If your page ships its shell as **static
HTML** — a sidebar, a header, the chat's tags in `index.html` — load the stylesheet
from the head instead, with a `<link>` or an `@import` at the top of your own CSS:
in development, Vite injects a JavaScript-imported sheet only after the module graph
has loaded, and the shell paints bare for a moment on every reload. A production build
extracts it into a `<link>` either way; the difference is what you see while you work.
:::
...
The fastest way to see a reply stream is a model that needs no key and no network.
`@aparte/provider-scenario` is a real `AparteAIProvider` that replays turns you wrote —
text at a typing pace, thinking, tool calls, errors. Install it, register it, start the
client:

```bash
npm install @aparte/provider-scenario
```
...
// `showcase` is a ready-made set of turns: a haiku, a table, some code, the weather
// (a real tool round-trip), an artifact.
...
<Demo
    ...
    caption="A real reply, streaming, with no key and no backend: the scripted model. Ask it for a haiku, a table, some code or the weather."
/>

Swap `createScenarioProvider` for a provider that talks to a real model and the rest of
this page is unchanged — that is [Wire a real model](#wire-a-real-model), below.

### Streaming it yourself, with no provider

If the reply comes from something aparté does not talk to — your own SSE endpoint, a
worker, a websocket — drive the viewport by hand. The composer fires an
**`aparte-send`** event when the user submits. It bubbles, so you can listen on
`<aparte-chat>`, and `chat.viewport` is the message list. Three methods are all you
need:
...
That is the shape [Bring your own loop](/guides/bring-your-own-loop/) develops. The rest
of this page assumes the client above.
...
The client owns the **whole** exchange: it echoes your **user** message too, files
included. If you wrote the `aparte-send` handler from
[Streaming it yourself](#streaming-it-yourself-with-no-provider), drop it — the two would
both answer the same send. A host that keeps its own transcript passes
...
- **[Layout](/guides/layout)** and **[the app shell](/guides/app-shell)** — the five shapes a chat takes, and the sidebar, header and grid of a whole page.
