...
`@aparte/engine` bundled into core's. One exception, and it is the only one:
[`@aparte/provider-transformers`](/providers/ai/transformers/) needs a bundler — its
worker cannot be built this way yet, and its page says why.
