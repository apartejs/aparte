...
included — so once it runs, drop the `aparte-send` handler from
[above](#make-it-stream) (that handler is the bring-your-own-loop path's job). A host
that keeps its own transcript passes `echoUserMessage: false`.
