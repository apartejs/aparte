...
| `parseMarkdownToSegments` | The one-shot version of the above, for a complete reply | You want the segments in hand before they reach a bubble — to store them, inspect them, or edit them. Not to render: `appendMessage` and `setMessages` already split an assistant's markdown string this way |
...
let waiting = false;                            // your own wait indicator, e.g. a spinner
const setWaiting = (on: boolean) => { waiting = on; };
...
const host = new AparteChatHost({
  ...
  onLoadingChange: (on) => setWaiting(on),     // draw your own wait; the viewport's `loading` attribute is set either way
});
...
### Structure a reply without rendering it

`parseMarkdownToSegments` is core's zero-dependency markdown structurer: a complete reply
in, typed segments out — text, fenced code with its language, thinking blocks, and the
block grammars you hand it. It reads no config of its own, so a grammar you registered
with `registerStreamBlock` reaches it only through the `blocks` option. No plugin, no
`marked`, no DOM. It is what you want *around* the chat: the code blocks a reply
contained, a reply stored as structure rather than as a string, two replies diffed
segment by segment.

```ts
import { aparteGlobalConfig, parseMarkdownToSegments, type AparteStreamParserOptions } from '@aparte/core';

// The same options the streaming parser takes.
const options: AparteStreamParserOptions = {
  thinkingDelimiters: { start: '<think>', end: '</think>' },
  blocks: aparteGlobalConfig.getStreamBlocks(),   // the grammars registered on the config
};

const segments = parseMarkdownToSegments('Here is the fix:\n\n```ts\nexport const x = 1;\n```\n', options);
const code = segments.filter((segment) => segment.type === 'code');
void code;
```
...
part of the reply. The delimiters are read on every path — while a reply streams and
when a stored reply is loaded again — so a message that merely quotes `<think>` in its
prose is read as reasoning too, and reasoning is not sent back to the model.
