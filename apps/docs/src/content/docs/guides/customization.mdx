---
title: Customization
description: Replace any region's markup, add action-bar buttons, render your own segment types (custom message content — generative UI), and swap icons — all without forking the components.
sidebar:
  order: 3
---
...
<Demo id="customization-bubble-shell" caption="The same message twice: the first is core's bubble, the second a shell of your own. One setBubbleShellRenderer, scoped to the second bubble with an instance config." />
...
Buttons on the message bubble come from a **registry**. Add your own with
`registerAction`:
...
- `zones` decides where it shows — `'bubble'` is the one zone; `bubble: { roles: [...] }`
  refines placement; `order` sorts custom actions.
- An `onClick(event)` callback is optional and fires alongside the event.
- Remove one with `aparteGlobalConfig.unregisterAction(id)`.
...
### A button in the composer

A composer button is not a registry entry — it is an element you write in your own
markup, `<aparte-composer-action>`, placed exactly where you want it in the row. It
emits `aparte-action-click`, and `action-id` is what tells two of them apart: it comes
back on the event's detail, and a second button without one is indistinguishable from
the first.
...
`icon` takes one of core's own glyph names (`copy`, `edit`, `pin`, `retry`, `trash`, …) or
a raw `<svg>…</svg>` string, which is how you bring your own. A name core does not know is
rendered **as text** — `icon="star"` puts the word *star* in the button — so an icon set of
your own goes through `setIconProvider`, not through the attribute.

Prefer the dedicated element wherever one exists — `<aparte-composer-send>`,
`<aparte-composer-cancel>`, `<aparte-composer-add-attachment>` — since those already talk
to the composer.
...
```ts
// You run an AparteClient, so retry and edit do something. Feedback and the details
// button are yours to answer, and the two listeners below are what answering them is:
aparteGlobalConfig.setBubbleActions({ retry: true, edit: true, feedback: true, info: true });

document.addEventListener('aparte-feedback', (e) => {
  const { messageId, value } = e.detail;          // value: 'positive' | 'negative'
  void fetch('/api/feedback', { method: 'POST', body: JSON.stringify({ messageId, value }) });
});

document.addEventListener('aparte-message-info', (e) => {
  openMyDetailsPopover(e.detail.messageId);       // e.detail.usage carries the numbers
});

// You handle this event yourself:
aparteGlobalConfig.setHostHandlers({ attachmentPreview: true });
```

Both listeners sit on `document` because the events bubble and compose: one listener
answers every chat on the page. The type behind the button names is
`AparteBubbleActionName`, so a per-role list is checked at compile time —
`setBubbleActions({ assistant: ['copy', 'retry', 'info'] satisfies AparteBubbleActionName[] })`.

The download on a *binary* artifact used to be a third row here; it is
[`@aparte/plugin-artifacts`](/plugins/artifacts/)'s now, and it follows the same rule in the
plugin's own terms — the button exists only when the app gave `setupArtifacts` an `onBinary`
that can produce the bytes.
...
No slot to fill: core has no shadow root, so the toolbar is an ordinary child you write
where you want it.
...
```vue
<AparteChat :messages="chat.messages.value" @messages-change="chat.onMessagesChange">
  <template #toolbar>
    <ModePicker v-model="mode" />
    <ModelSelector style="margin-inline-start: auto" />
  </template>
</AparteChat>
```
...
A fragment, so several nodes project without a wrapper element around them.

```svelte
<AparteChat messages={$messages} on:messagesChange={handleMessagesChange}>
  <svelte:fragment slot="toolbar">
    <ModePicker bind:value={mode} />
    <ModelSelector style="margin-inline-start: auto" />
  </svelte:fragment>
</AparteChat>
```
...
`slot="toolbar"` on each projected node — Angular projects by selector, so the attribute
goes on the node itself rather than on a wrapper.

```html
<aparte-chat [messages]="messages" (messagesChange)="messages = $event">
  <app-mode-picker slot="toolbar"></app-mode-picker>
  <aparte-model-selector slot="toolbar" style="margin-inline-start: auto"></aparte-model-selector>
</aparte-chat>
```
...
source: [Wrapper surface](/reference/wrappers/).
...
The methods the viewport will call on that element are optional, and the set is the one
`SyncableBubble` declares — `getSegments` / `setSegments` / `updateSegment` /
`setContent` / `setAttachments` / `setSiblings` / `setUsage`. Implement what you need.

Two caveats the snippet cannot show. `clearAll()` removes `<aparte-chat-bubble>` nodes
only, so an element of your own is left behind with nothing left to render. And any
branch operation — `addBranch`, `addSiblingOf`, `navigateBranch`, `importTree` — empties
the transcript and rebuilds it from the repository, so only what the repository holds
comes back. In a framework, the custom-bubble hook the wrapper gives you — `renderBubble`
in React, the `bubble` slot in Vue and Svelte, `[bubbleTemplate]` in Angular — is the
supported path.
...
One renderer answers for **every** `custom` segment: the registry is keyed on
`renderer.type`, not on `subType`. So a renderer for `custom` switches on `subType`
itself, and two independent packs cannot each own one — pick a prefix if you ship a pack.

```ts
// registerSegmentRenderer, AparteSegmentRenderer and AparteCustomSegment come from '@aparte/core'.
const custom: AparteSegmentRenderer<AparteCustomSegment> = {
  type: 'custom',
  render(segment) {
    switch (segment.subType) {
      case 'weather-widget': return renderWeather(segment.data);
      case 'onboarding': return renderOnboarding(segment.data);
      default: return segment.fallback ?? '';
    }
  },
};

registerSegmentRenderer(custom);
```

Writing a renderer in a module of its own — which is what a pack is — is what
`AparteSegmentRenderer<T>` is for.

### Teach the parser a block

A renderer draws a segment; something has to *produce* it. Models write conventions into
their prose — `<think>` for reasoning, `<artifact type="…">` for a document, `<file path="…">`
for a patch, `<cite url="…">` for a source — and every one is the same shape: an opening tag
with attributes, a body that streams, a closing tag. Register the grammar and the stream
parser does the rest:

```ts
import { aparteGlobalConfig, registerSegmentRenderer } from '@aparte/core';
import type { AparteSegmentBase, AparteStreamBlockMatch } from '@aparte/core';

interface CitationSegment extends AparteSegmentBase {
  type: 'citation';
  url: string;
  content: string;
}
...
aparteGlobalConfig.registerStreamBlock({
  tag: 'cite',
  toSegment: ({ attrs, id }: AparteStreamBlockMatch): CitationSegment =>
    ({ id, type: 'citation', url: attrs['url'] ?? '', content: '' }),
});

registerSegmentRenderer<CitationSegment>({
  type: 'citation',
  render(segment) {
...
`toSegment` is called once, at the opening tag, with an `AparteStreamBlockMatch`: the
attributes as written (quoted or bare — `<cite url="…">`, `<cite url=…>`) and the id the
parser generated. It returns the segment with an empty `content`, and the parser streams
the body into it delta by delta,
marks it `isStreaming` while the block is open and closes it at `</cite>` or at the end of
the stream. Everything that makes this hard is done once for every grammar: the earliest
tag wins against a code fence and a reasoning delimiter, a tag cut at a chunk boundary is
held back, a closing tag split across two chunks never leaks as content, and `<cites>` does
not open a `cite` block. What the segment *is* stays yours — a `type` with a renderer, or a
built-in one if the block is only another spelling of it. The blocks are read when a turn
starts, so a grammar registered mid-turn applies from the next one.
...
function writeDuration(host: HTMLElement, segment: AparteThinkingSegment): void {
  ...
  // A sub-second span is "<1s", not "0.0s" — a zero reads as "did not think".
...
Core carries no third-party dependency, so Markdown and syntax highlighting are **off**
...
:::note[Providers do not inherit; renderers do]
An instance config starts from the built-in defaults — it does **not** inherit the
providers registered on `aparteGlobalConfig` (markdown, highlight, icons, locale,
transport, AI providers). Register what each instance needs on that instance.

The **segment-renderer registry is the exception, by design**: an instance config
inherits every renderer registered globally, so a page-wide `registerSegmentRenderer`
reaches a scoped chat too, and a config that declined the built-ins inherits nothing.
:::

:::caution[An instance cannot subtract]
Inheritance is one-way. There is no tombstone, so an instance config **cannot suppress a
type the global registry declares** — two chats on one page, one with a pack of renderers
and one without, is not expressible if the pack registered globally. Register the pack on
the instance that wants it instead.
:::
