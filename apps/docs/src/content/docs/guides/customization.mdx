...
| re-generating a *persisted* binary artifact | your generator (`aparte-artifact-ready`, on mount) | off |
...
The last row is off for a stronger reason than the others, and worth reading before you
turn it on: `artifactRehydrate` is an automatic dispatch nobody asked for, carrying
model-authored content the receiving app is expected to run. Re-opening a saved
conversation would re-execute whatever a prompt injection had persuaded the model to
persist — on every reload. The full note is in the
[config reference](/reference/config/#host-handlers).
