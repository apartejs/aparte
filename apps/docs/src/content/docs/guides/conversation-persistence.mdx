---
title: Persist chat conversations (IndexedDB, SQLite, REST)
...
interface AparteStorageAdapter {
  ...
  //   pin?/unpin?, rename?, and loadAttachments? for persisted attachment blobs.
}
```
...
and optional `archivedAt` / `pinnedAt` / `schemaVersion` (current version is
...
and `loadAttachments(msgId)` for attachment blobs — all optional, consulted only when
present. Anything beyond the conversation — a memory of user facts, a settings store, a
gallery of every artifact ever produced — is your app's own table: extend the interface
in your code rather than expecting the library to carry your schema.
...
There is no binding without a wrapper — the pair is a wrapper convenience over
`AparteConversationController`, which is the next section on this page.
...
```vue
<AparteChat :conversation-id="conversationId" @conversation-created="conversationId = $event" />
```
...
```svelte
<AparteChat {conversationId} on:conversationCreated={(e) => (conversationId = e.detail)} />
```
...
<aparte-chat
  ...
  (conversationCreated)="conversationId = $event"
></aparte-chat>
```
...
const controller = new AparteConversationController({
  ...
  setLoading: (on) => viewport.setLoading(on), // optional — the viewport draws the wait
...
### A conversation on its way

A store takes its time. When your adapter implements `loadMeta()` and `loadFull(id)`,
the manager reads the list through the first and fetches a conversation's messages
through the second the first time it is opened (`manager.ensureFull(id)`, and
`manager.isLoaded(id)` says whether it has). While that fetch runs, the controller calls
the binding's `setLoading(true)`: `<aparte-chat-viewport>` draws two skeleton turns,
marks its scroll surface `aria-busy` and names the wait for a screen reader, and
`aparte-chat[center-empty]` does not count it as empty — the welcome never shows over a
conversation that is loading. The composer is disabled for the duration — a message sent
into a transcript that has not arrived would be overwritten by it — and comes back as
it was (a `disabled` you set stays). Two quick clicks are safe: a reply that lands for a
conversation the user has already left is dropped. `ensureFull()` resolves `false` when the store
has no such record — the row then does not count as loaded, and the controller goes home rather
than keep an empty transcript active. A metadata change (`updateTitle`, `pin`, `archive`) on a
conversation whose messages are not in memory goes through the adapter's own hook when it has one
(`rename()`, `pin()`, `archive()`…) and otherwise fetches the messages first, so a row the list came
with is never written back without them; `updateMessages()` on such a row is refused with a warning.
A metadata change leaves `updatedAt` alone — a rename, a pin and an archive keep the conversation's
place in the list, which sorts on the last message rather than on the last edit.

The list has the same word for its own wait: set `loading` on
`<aparte-conversation-list>` while `manager.init()` runs, clear it when you assign
`conversations`.

The framework wrappers draw the same wait themselves — the controller sets it while it
fetches — and each has a `loading` prop (an input in Angular) for a wait of your own.

## 4. Render a sidebar

`<aparte-conversation-list>` is a display primitive: set its `conversations` property and
`active-id` attribute. Each row is its title plus a `⋯` menu: rename, pin/unpin,
archive/unarchive, delete (which asks first). The rows are grouped by date as soon as any
item carries `updatedAt`; set `flat` to render them flat.

Add `manage` and the menu writes through to the manager you registered in step 2 — those
gestures need no code of yours at all:

```ts
const list = document.querySelector('aparte-conversation-list')!;

function render() {
  list.conversations = manager.activeConversations;
  list.setAttribute('active-id', manager.activeId ?? '');
}
manager.subscribe(render);
render();

// The two lines that finish the row menu — the first is step 2's, repeated here so the
...
// `aparte-conversation-select` event that every bound controller listens for), not the list.
```

**When you own the persistence**, leave `manage` off: the list then only announces the
intent, and every gesture is yours to carry out. The events all bubble, with
`detail: { id }` — rename adds `title`. They fire under `manage` too, first and
cancelable, so `preventDefault()` on one takes that single gesture back and leaves the
rest to the list.
...
list.addEventListener('aparte-conversation-unpin', (e) => manager.unpin(e.detail.id));
list.addEventListener('aparte-conversation-archive', (e) => manager.archive(e.detail.id));
list.addEventListener('aparte-conversation-unarchive', (e) => manager.unarchive(e.detail.id));
...
`createNew` / `addMessage` / `updateMessages` / `updateTitle` / `pin` / `unpin` / `delete` /
`archive` / `unarchive` to actions:
...
back. One exception, and it is core's own: the `segments` core derived by splitting a
markdown reply on display are not written back — neither in `messages` nor in the branch
tree. They are core's reading of your `content`, so they are read again on the next open
and a block grammar you register later applies to conversations already saved.

Core then **adopts** the segments you did store rather than treating them as new — and the difference is
