...
item carries `updatedAt`; set `no-groups` to render them flat.
...
list.addEventListener('aparte-unpin-conversation', (e) => manager.unpin(e.detail.id));
list.addEventListener('aparte-archive-conversation', (e) => manager.archive(e.detail.id));
list.addEventListener('aparte-unarchive-conversation', (e) => manager.unarchive(e.detail.id));
