...
Transport errors now say what happened: a non-ok response from `AparteDirectTransport` or `AparteBackendTransport` throws an `AparteError` with the vendor's message, `httpStatus`, and a `code` read off the status — `429` → `USAGE_RATE_LIMIT`, `401`/`403` → `CONFIG_INVALID_KEY` (new code), `503` → `PROVIDER_UNAVAILABLE`, other `5xx` → `PROVIDER_ERROR`, `400` → `USAGE_BAD_REQUEST`, `408` → `NET_TIMEOUT`. Until now every one of them reached the error card and `aparte-message-error` as `UNKNOWN_ERROR`, whatever the vendor had said; a listener that matched `code === 'UNKNOWN_ERROR'` to catch transport failures should match the new codes (or the class) instead.
