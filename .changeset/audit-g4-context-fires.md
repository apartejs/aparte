...
`<aparte-context>` declares the `aparte-compact` event it dispatches, so it appears in the shipped custom-elements manifest, on the element's docs page, and in the editor tooltip that manifest feeds.
